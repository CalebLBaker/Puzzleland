 ...
 *
 *
 *	Modified: August 17, 2017 by Caleb Baker
 *	Altered print function to allow for faster printing.
*/


#include <cstdio>		// For printf.
#include <cstdint>		// For uint16_t and the like.
#include <windows.h>	// For editing terminal settings.
#include <unistd.h>
#include <iostream>
...
void print() {
	...
	// Print the room.
	char toPrint[3241];
	toPrint[height * (width + 1)] = '\0';
	for (uint8_t i = 0; i < height; i++) {
		toPrint[i * (width + 1) - 1] = '\n';
		for (uint8_t j = 0; j < width; j++) {
			if ((flags & 0x20) == 0x20 && board[width * i + j] == 'X') {
				toPrint[i * (width + 1) + j] = 'Y';
				// printf("Y");
			}
			else {
				toPrint[i * (width + 1) + j] = board[width * i + j];
				// printf("%c", board[width * i + j]);
			}
		}
	}
	std::cout << toPrint << '\n';	// Ugly though it's syntax may be, cout does print faster than printf and I need this to print fast.
	// printf("%s\n", toPrint);
}
...
int main() {
	...
	uint16_t (*initialize[121])(char, void (**)(uint32_t*, uint16_t), uint32_t*) =  {	nullptr,	nullptr,	nullptr,	topLeft,	top,	top,	top,	top,	top,		topRight,		nullptr,
																						...
																						nullptr,	wallOdeath,	warpPoint,	vert,		left,	room,	cheese,	room,	room,		right,			nullptr,
																						nullptr,	bottomLeft,	copyCats,	tele,		left,	room,	room,	room,	room,		right,			nullptr,
